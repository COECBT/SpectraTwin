...
def _load_keras():
    ...
    return {
        "Sequential": Sequential, "Dense": Dense, "Conv1D": Conv1D,
        "MaxPooling1D": MaxPooling1D, "Flatten": Flatten, "Dropout": Dropout,
        "BatchNormalization": BatchNormalization, "Adam": Adam,
        "RMSprop": RMSprop, "SGD": SGD, "EarlyStopping": EarlyStopping,
    }
...
class Models:
    ...
    def _manual_hp_specs(self, is_classification=False):
        if is_classification:
            return {
                "Logistic Regression": (LogisticRegression, {"max_iter": 1000}, [
                    ("C", "float", 1.0, None),
                    ("solver", "select", "lbfgs", ["lbfgs", "liblinear", "newton-cg", "sag", "saga"]),
                ]),
                ...
                "AdaBoost": (AdaBoostClassifier, {"random_state": 42}, [
                    ("n_estimators", "int", 50, None),
                    ("learning_rate", "float", 1.0, None),
                ]),
                "SVR Classifier": (SVC, {"probability": True}, [
                    ("kernel", "select", "rbf", ["linear", "poly", "rbf", "sigmoid"]),
                    ("C", "float", 1.0, None),
                    ("gamma", "select", "scale", ["scale", "auto"]),
                ]),
                ...
                "KNN Classifier": (KNeighborsClassifier, {}, [
                    ...
                    ("weights", "select", "uniform", ["uniform", "distance"]),
                    ("algorithm", "select", "auto", ["auto", "ball_tree", "kd_tree", "brute"]),
                ]),
        ...
        return {
            ...
            "Ridge Regression": (Ridge, {}, [
                ("alpha", "float", 1.0, None),
                ("solver", "select", "auto", ["auto", "svd", "cholesky", "lsqr", "sparse_cg", "sag", "saga"]),
                ("max_iter", "int", 1000, None),
                ("tol", "float", 0.001, None),
            ]),
            "ElasticNet Regression": (ElasticNet, {}, [
                ("alpha", "float", 1.0, None),
                ("l1_ratio", "float", 0.5, None),
                ("max_iter", "int", 1000, None),
            ...
            "AdaBoost": (AdaBoostRegressor, {"random_state": 42}, [
                ("n_estimators", "int", 50, None),
                ("learning_rate", "float", 1.0, None),
                ("loss", "select", "linear", ["linear", "square", "exponential"]),
            ]),
            "SVR": (SVR, {}, [
                ("kernel", "select", "rbf", ["linear", "poly", "rbf", "sigmoid"]),
                ("C", "float", 1.0, None),
    ...
    def render_hyperparameters(self, model_name, is_classification=False, key_prefix=""):
        """Render Streamlit widgets for a model's hyperparameters and return the
        chosen values as a dict. Call this OUTSIDE the train button so the
        widgets persist and stay editable."""
        specs = self._manual_hp_specs(is_classification).get(model_name)
        if not specs:
            return {}
        _, _, param_list = specs
        if not param_list:
            st.caption("This model has no tunable hyperparameters.")
            return {}

        st.markdown("**Hyperparameters** (edit before training)")
        values = {}
        cols = st.columns(2)
        for i, (name, kind, default, options) in enumerate(param_list):
            container = cols[i % 2]
            widget_key = f"hp_{key_prefix}_{model_name}_{name}"
            label = name.replace("_", " ").title()
            if kind == "bool":
                values[name] = container.selectbox(label, [True, False], index=0 if default else 1, key=widget_key)
            elif kind == "int":
                values[name] = int(container.number_input(label, value=int(default), step=1, min_value=1, key=widget_key))
            elif kind == "float":
                values[name] = float(container.number_input(label, value=float(default), min_value=0.0,
                                                             format="%.5f", key=widget_key))
            elif kind == "select":
                idx = options.index(default) if default in options else 0
                values[name] = container.selectbox(label, options, index=idx, key=widget_key)
        return values

    def build_estimator(self, model_name, is_classification, params, y_train):
        """Construct (and multi-output-wrap) a model from chosen hyperparameters."""
        specs = self._manual_hp_specs(is_classification).get(model_name)
        if not specs:
            raise ValueError(f"Unknown manual model: {model_name}")
        estimator_cls, fixed_kwargs, _ = specs
        kwargs = dict(fixed_kwargs)
        for k, v in (params or {}).items():
            kwargs[k] = None if v == "None" else v
        model = estimator_cls(**kwargs)
        if is_classification:
            return self.wrap_classifier_if_needed(model, y_train)
        return self.wrap_regressor_if_needed(model, y_train)
    ...
    def Random_forest_classifier(self, X_train, X_test, y_train, y_test):
        ...
        min_samples_leaf = self.get_param("Min samples leaf", 1, int)
        criterion = self.get_param("Criterion (gini, entropy)", "gini", str)
        max_features = self.get_param("Max features (sqrt/log2/None)", "sqrt", lambda x: None if x.strip().lower() == "none" else x.strip())
        bootstrap = self.get_param("Bootstrap (True/False)", True, lambda x: x.lower() == 'true')
...
#regression cases
    ...
    def ANN_regression(self, X_train, X_test, y_train, y_test):
        ...
        RMSprop = _k["RMSprop"]
        SGD = _k["SGD"]
        EarlyStopping = _k["EarlyStopping"]
...
class AutoModelSelector:
    def __init__(self, task="regression", scoring_fn=None, verbose=True, n_trials=20, per_model_timeout=120):
        ...
        try:
            ...
            pass
        self.model_class = optuna_Model(n_trials=n_trials, timeout=per_model_timeout)
...
class OutlierRemover:
    ...
    def transform(self, X):
        ...
        Records which rows were kept (``kept_mask_`` / ``kept_indices_``) and
        which were dropped (``outlier_indices_``) so that the corresponding
        target rows can be filtered the same way.
        """
        spectra = X.values
        mask = np.all((spectra >= self.lower) & (spectra <= self.upper), axis=1)
        self.kept_mask_ = mask
        self.kept_indices_ = np.where(mask)[0]
