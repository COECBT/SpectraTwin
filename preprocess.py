...
class SpectralData(Sequence):
    ...
    def __init__(self, file=None):
        ...
        if file is None:
            ...
            # Spectral columns must be numeric wavelengths. If non-numeric
            # columns slip through (e.g. a sample-ID or class/label column that
            # was not dropped), the wavelength sort and the spacing arithmetic
            # below fail with a cryptic
            # "'<' not supported between instances of 'str' and 'float'".
            # Coerce column labels to numeric and drop any that are not.
            col_numeric = pd.to_numeric(pd.Series(list(combined.columns), dtype=object),
                                        errors='coerce')
            keep_mask = ~col_numeric.isna().values
            if not keep_mask.any():
                raise ValueError(
                    "No numeric spectral (wavelength) columns were found in the input. "
                    "Make sure non-spectral columns (sample IDs, labels, targets) are "
                    "removed before preprocessing."
                )
            if not keep_mask.all():
                combined = combined.loc[:, keep_mask]
                col_numeric = col_numeric[keep_mask]
            combined.columns = col_numeric.values
    ...
    def readTextFile(self, file):
        ...
        # Decide orientation by detecting which axis holds numeric wavelengths
        # rather than by comparing row/column counts. The old shape-based rule
        # ("more rows than columns -> rows are wavelengths") silently transposed
        # and scrambled any dataset with more samples than wavelengths (n > p).
        def _numeric_fraction(values):
            s = pd.to_numeric(pd.Series(list(values)), errors='coerce')
            return float(s.notna().mean()) if len(s) else 0.0

        def _is_monotonic_wavelength_axis(values):
            # The wavelength axis is numeric and sorted; spectra (intensities)
            # are generally not monotonic, so monotonicity reliably identifies
            # the wavelength axis even when the CSV header was not detected.
            s = pd.to_numeric(pd.Series(list(values)), errors='coerce')
            if len(s) < 3 or s.notna().mean() < 0.8:
                return False
            s = s.dropna()
            return bool(s.is_monotonic_increasing or s.is_monotonic_decreasing)

        oriented = False
        if has_header and raw.shape[1] > 0:
            col_numeric = _numeric_fraction(raw.columns)
            firstcol_numeric = _numeric_fraction(raw.iloc[:, 0])

            if col_numeric >= 0.8:
                # Column headers are the (numeric) wavelength axis -> rows are
                # spectra/samples already. Keep as-is regardless of n vs p.
                oriented = True
            elif firstcol_numeric >= 0.8:
                # Wavelengths live in the first column (one spectrum per other
                # column); make them the columns.
                raw = raw.set_index(raw.columns[0]).transpose()
                oriented = True

        if not oriented and raw.shape[0] > 0 and raw.shape[1] > 0:
            # Header not reliably detected (common with all-numeric spectra).
            # Use monotonicity to find the wavelength axis.
            if _is_monotonic_wavelength_axis(raw.iloc[0]):
                # First row is the wavelength header that was read as data.
                new_cols = pd.to_numeric(raw.iloc[0], errors='coerce')
                raw = raw.iloc[1:].reset_index(drop=True)
                raw.columns = new_cols
                oriented = True
            elif _is_monotonic_wavelength_axis(raw.iloc[:, 0]):
                # First column is the wavelength axis (wavelengths in rows).
                raw = raw.set_index(raw.columns[0]).transpose()
                oriented = True

        if not oriented:
            # Truly ambiguous: fall back to the original shape-based heuristic.
